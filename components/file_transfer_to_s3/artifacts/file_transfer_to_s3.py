...
class MyHandler(FileSystemEventHandler):
    def process(self, event):
        ...
        if event.event_type == 'created' and os.path.isfile(file_path):  # Only process file created events
            print(f"Detected new file: {file_path}")
            
            s3 = boto3.client('s3')
            bucket_name = 'com.enovis.duo-poc'
            ...
            try:
                with open(file_path, 'rb') as data:
                    s3.upload_fileobj(data, bucket_name, key, ExtraArgs={'ContentType': 'application/pdf'})
